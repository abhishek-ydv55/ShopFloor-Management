...
@Component({
    ...
    <div class="app-shell">
      ...
      <div class="main-content">
        <div class="page-content">
          ...
          <!-- External Procurement Banner -->
          @if (sentToExternalRequests().length > 0) {
            <div class="ext-proc-banner">
              <span class="ext-proc-banner__icon">&#8599;</span>
              <div class="ext-proc-banner__body">
                <span class="ext-proc-banner__title">
                  {{ sentToExternalRequests().length }} item{{ sentToExternalRequests().length > 1 ? 's' : '' }} sent for external procurement
                </span>
                <span class="ext-proc-banner__detail">{{ getExternalProcItemNames() }} — awaiting vendor fulfillment</span>
              </div>
              <button class="ext-proc-banner__btn" (click)="activeTab.set('spare')">View →</button>
            </div>
          }
            ...
            <app-card>
              <div class="section-header" style="flex-wrap:wrap; gap:10px;">
                ...
                <div style="display:flex; gap:8px; align-items:center; flex-wrap:wrap;">
                  ...
                  <button class="sort-crit-btn" [class.sort-crit-btn--active]="sortByCriticality()"
                    (click)="sortByCriticality.update(v => !v)"
                    title="Sort items by stock criticality (qty ÷ threshold) — most critical first">
                    {{ sortByCriticality() ? '&#8593; Criticality' : '&#8645; Criticality' }}
                  </button>
  ...
  styles: [`
    ...
    .ext-proc-banner {
      ...
      border-left: 4px solid #f59e0b; border-radius: var(--radius-md);
      padding: 14px 16px; margin-bottom: var(--space-5);
    }
    .ext-proc-banner__icon { font-size: 20px; color: #d97706; flex-shrink: 0; }
    .ext-proc-banner__body { flex: 1; min-width: 0; }
    .ext-proc-banner__title { display: block; font-weight: 700; font-size: var(--font-sm); color: #92400e; }
    .ext-proc-banner__detail { display: block; font-size: var(--font-xs); color: #b45309; margin-top: 2px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
    .ext-proc-banner__btn {
      background: #f59e0b; color: #fff; border: none;
      border-radius: var(--radius-sm); padding: 6px 14px;
      font-size: var(--font-sm); font-weight: 600; cursor: pointer;
      transition: background var(--transition); flex-shrink: 0;
    }
    .ext-proc-banner__btn:hover { background: #d97706; }

    .sort-crit-btn {
      padding: 6px 14px; border: 1.5px solid var(--gray-border);
      border-radius: var(--radius-md); background: var(--white);
      font-size: var(--font-sm); font-weight: 600; color: var(--text-dark);
      cursor: pointer; transition: all var(--transition); white-space: nowrap;
    }
    .sort-crit-btn:hover { border-color: var(--red-700); color: var(--red-700); }
    .sort-crit-btn--active { background: var(--red-700); color: #fff; border-color: var(--red-700); }
  `]
})
export class InventoryComponent implements OnInit {
  ...
  // ── Stock search & criticality sort ─────────────────────────────────────
  stockSearch       = signal('');
  sortByCriticality = signal(false);
  ...
  filteredItems = computed(() => {
    ...
    let list = q
      ...
      : this.items();
    if (this.sortByCriticality()) {
      list = [...list].sort((a, b) => (a.qty / a.threshold) - (b.qty / b.threshold));
    }
    return list;
  });
  ...
  sentToExternalRequests = computed(() =>
    this.spareRequests().filter(r => r.status === 'SentToExternal')
  );

  getExternalProcItemNames(): string {
    const names = [...new Set(this.sentToExternalRequests().map(r => r.item))];
